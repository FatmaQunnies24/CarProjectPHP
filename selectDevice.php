...
<section>
   ...
   <p> <br><br><br></p>
        ...
        <div class="form-box">
            <h2>Update , Insert ,search device</h2>
            <form id="updateForm">
...
<select id="carNames" name="updateCar">
...
<select id="carNames" name="insertCar">
    <option value="" >made</option>
    <?php
    while ($rowCar = $carNamesQuery->fetch_assoc()) {
        echo "<option value='" . $rowCar["name"] . "'>" . $rowCar["name"] . "</option>";
    ...
    <button type="submit" id="btn">Insert Car</button>
            ...
            <div class="search-container">
                <input type="text" id="carName" placeholder="no">
...
if (!empty($insertCarName) && !empty($insertCarModel) && !empty($insertCarYear) && !empty($insertCarMade) && !empty($insertCar)) {
    ...
    $checkDuplicateQuery = "SELECT * FROM Device WHERE no = '$insertCarName'";
    ...
    if ($checkDuplicateResult !== false) {
        ...
        if (mysqli_num_rows($checkDuplicateResult) > 0) {
            ...
            $sql = "INSERT INTO Device (no, name, price, weight, made) VALUES ('$insertCarName', '$insertCarModel', '$insertCarYear', '$insertCarMade', '$insertCar')";
...
if (!empty($updateCarName) && !empty($updateCarModel) && !empty($updateCarYear) && !empty($updateCarMade) && !empty($updateCar)) {
    // Check if the primary key already exists
    $checkDuplicateQuery = "SELECT * FROM Device WHERE no = '$updateCarName'";
    $checkDuplicateResult = mysqli_query($conn, $checkDuplicateQuery);

    if ($checkDuplicateResult) {
        // Check if any rows were returned
        if (mysqli_num_rows($checkDuplicateResult) > 0) {
            // Performing update query execution(no, name, price, weight, made)
            $sql = "UPDATE Device SET name = '$updateCarModel', price = '$updateCarYear', weight = '$updateCarMade', made = '$updateCar' WHERE no = '$updateCarName'";
